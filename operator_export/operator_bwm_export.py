"""
Main module for the export plugin handle necessary code for its registration
...
def write_bwm_data(
    context: bpy.types.Context, filepath: str, settings: Dict[str, Any]
):
    """
    Call the function to translate a blender collection into a .bwm file then
    write the file
    """
...
class BwmDataPanel(BwmGenericPanel):
    ...
    def draw(self, context: bpy.types.Context):
        ...
        layout.label(text=selected_name, translate=False, icon=selected_icon)
        if valid == "OPT_VALID":
            layout.label(
        ...
        if valid == "OPT_INVALID":
            layout.label(
                text="No export possible", translate=False, icon="PANEL_CLOSE"
            )
        if valid == "OPT_WARNING":
            ...
            for col_name in inv_col:
                column.label(
                    text=col_name, translate=False, icon="OUTLINER_COLLECTION"
                )


def menu_func_export(self, context):
    self.layout.operator(
        ExportBWMData.bl_idname, text="Black & White Model (.bwm)"
    )


classes = (ExportBWMData, BwmDataPanel)
